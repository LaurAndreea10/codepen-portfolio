...
@media (max-width: 900px), (hover: none) and (pointer: coarse) {
  ...
  .hero-preview-wrap,
  .hero-preview-frame { min-height: 0; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  function addCompletedProjects() {
    ...
    const projects = [
      { title: 'FileVerse 2.0', text: isEnglish ? 'CodePen 2.0 file-options challenge completed and published as a live GitHub Pages project.' : 'challenge-ul CodePen 2.0 dedicat opțiunilor pentru fișiere a fost finalizat și publicat live pe GitHub Pages.', href: 'https://laurandreea10.github.io/CodePen-2.0-file-options-challenge/', label: isEnglish ? 'Open project' : 'Deschide proiectul' },
      { title: 'BlockForge — CodePen Challenge: Blocks', text: isEnglish ? 'interactive blocks challenge completed and published as a live GitHub Pages project.' : 'challenge-ul interactiv dedicat blocurilor a fost finalizat și publicat live pe GitHub Pages.', href: 'https://laurandreea10.github.io/BlockForge-CodePen-Challenge-Blocks/', label: isEnglish ? 'Open project' : 'Deschide proiectul' },
      { title: 'Elsewhere — CodePen Challenge: View Transitions', text: isEnglish ? 'cinematic atlas with portal transitions, day/night views, compare mode, mini-map and optional soundscapes.' : 'atlas cinematografic cu tranziții portal, mod zi/noapte, compare view, mini-hartă și soundscape opțional.', href: 'https://laurandreea10.github.io/CodePen-Challenge-View-Transitions/', label: isEnglish ? 'Open project' : 'Deschide proiectul' }
    ];
    const fragment = document.createDocumentFragment();
    for (const project of projects) {
      if (!list.querySelector(`[data-completed-project="${CSS.escape(project.title)}"]`)) fragment.append(completedItem(project));
    }
    if (fragment.childNodes.length) list.prepend(fragment);
    return projects.every(project => list.querySelector(`[data-completed-project="${CSS.escape(project.title)}"]`));
  }
  ...
  function addViewTransitionsCard() {
    ...
    card.innerHTML = `<span class="badge-new">NEW</span><div class="project-top"><div><h3>Elsewhere — View Transitions</h3><span class="badge-github">CodePen Challenge</span></div><span class="tag github">challenge</span></div><p class="project-desc">${isEnglish ? 'Cinematic destination atlas with shared-element morphs, click-position portal reveals, day/night scenes, interactive mini-map, compare mode and optional soundscapes.' : 'Atlas cinematografic cu shared-element morphs, portal circular din punctul de click, scene zi/noapte, mini-hartă interactivă, compare mode și soundscape opțional.'}</p><div class="card-actions"><a class="btn btn-primary" href="https://laurandreea10.github.io/CodePen-Challenge-View-Transitions/" target="_blank" rel="noopener noreferrer">Live Demo</a><a class="btn btn-secondary" href="https://github.com/LaurAndreea10/CodePen-Challenge-View-Transitions" target="_blank" rel="noopener noreferrer">GitHub &rarr;</a></div>`;
    grid.prepend(card);
    return true;
  }

  function refreshAdditions() {
    return updateNowDate() && addCompletedProjects() && addViewTransitionsCard();
  }
