...
function applyTranslations(){
  document.querySelectorAll('[data-i18n]').forEach(el=>{const k=el.dataset.i18n; if(translations[currentLang]?.[k]) el.textContent=translations[currentLang][k];});
  if(dom.langToggle) dom.langToggle.textContent=currentLang==='ro'?'RO | EN':'EN | RO';
  const fallbackText=byId('libraryFallbackText'),resetButton=byId('resetFiltersBtn'),latestMeta=byId('gh-meta-static');
...
function normalizeProject(raw,i=0){
  ...
  const title=typeof raw.title==='object'?{ro:raw.title.ro||raw.title.en||`Project ${id}`,en:raw.title.en||raw.title.ro||`Project ${id}`}:{ro:String(raw.title||`Project ${id}`),en:String(raw.title||`Project ${id}`)};
  const description=typeof raw.description==='object'?{ro:raw.description.ro||raw.description.en||'',en:raw.description.en||raw.description.ro||''}:{ro:String(raw.description||''),en:String(raw.description||'')};
...
function dedupeProjects(list){
  const map=new Map();
  list.forEach((p,i)=>{const n=normalizeProject(p,i); if(!n) return; const key=n.repoUrl||n.liveUrl||n.url||`${n.id}-${n.title.en}`; if(!map.has(key)) map.set(key,n);});
  return [...map.values()];
}
function createLibraryCard(project){
  const card=document.createElement('article'); card.className='project-card glass';
  const tag=document.createElement('span'); tag.className=`tag ${project.category||''}`.trim(); tag.textContent=project.category;
  const heading=document.createElement('h3'); heading.textContent=project.title[currentLang];
  const desc=document.createElement('p'); desc.className='project-desc'; desc.textContent=project.description[currentLang];
  const actions=document.createElement('div'); actions.className='card-actions';
  if(project.isNew){const b=document.createElement('span'); b.className='badge-new'; b.textContent=t('badge_new'); card.append(b);}
  const top=document.createElement('div'); top.className='project-top';
  const topLeft=document.createElement('div'); topLeft.append(heading);
  if(project.repoUrl){const gh=document.createElement('span'); gh.className='badge-github'; gh.innerHTML=`${GH_ICON}<span>GitHub</span>`; topLeft.append(gh);}
  top.append(topLeft,tag);
  if(project.liveUrl){const a=document.createElement('a'); a.className='mini-btn primary'; a.href=project.liveUrl; a.target='_blank'; a.rel='noopener noreferrer'; a.textContent=t('view_live'); actions.append(a);}
  if(project.liveUrlPro){const a=document.createElement('a'); a.className='mini-btn'; a.href=project.liveUrlPro; a.target='_blank'; a.rel='noopener noreferrer'; a.textContent=t('view_live_pro'); actions.append(a);}
  if(project.repoUrl){const a=document.createElement('a'); a.className='mini-btn'; a.href=project.repoUrl; a.target='_blank'; a.rel='noopener noreferrer'; a.innerHTML=`${GH_ICON}<span style="margin-left:6px">${t('view_repo')}</span>`; actions.append(a);}
  else {const a=document.createElement('a'); a.className='btn btn-secondary'; a.href=project.url; a.target='_blank'; a.rel='noopener noreferrer'; a.textContent=t('open'); actions.append(a);}
  card.append(top,desc,actions);
  return card;
}
function renderLatestGithub(){
  const container=byId('gh-projects-static'); if(!container) return;
  const source=projects.length?projects:STATIC_FALLBACK_GITHUB_PROJECTS;
  const githubProjects=source.filter(p=>(p.category||'').toLowerCase()==='github').sort((a,b)=>Number(!!b.isNew)-Number(!!a.isNew)||(b.id||0)-(a.id||0));
  if(!githubProjects.length){container.innerHTML=''; return;}
  const pageSize=3,totalPages=Math.max(1,Math.ceil(githubProjects.length/pageSize)); latestGithubPage=Math.min(latestGithubPage,totalPages-1);
  const pages=[]; for(let i=0;i<githubProjects.length;i+=pageSize) pages.push(githubProjects.slice(i,i+pageSize));
  container.innerHTML=''; container.classList.add('key-carousel');
  const viewport=document.createElement('div'); viewport.className='key-carousel-viewport';
  const track=document.createElement('div'); track.className='key-carousel-track';
  pages.forEach((pageProjects,pageIndex)=>{const slide=document.createElement('div'); slide.className='projects-grid'; slide.dataset.ghSlide=String(pageIndex); pageProjects.forEach(project=>slide.appendChild(createLibraryCard(project))); track.appendChild(slide);});
  viewport.appendChild(track); container.appendChild(viewport); track.style.transform=`translateX(-${latestGithubPage*100}%)`;
  let controls=byId('gh-projects-pagination');
  if(!controls){controls=document.createElement('div'); controls.id='gh-projects-pagination'; controls.className='key-carousel-controls'; container.insertAdjacentElement('afterend',controls);}
  if(totalPages<2){controls.innerHTML=''; controls.hidden=true; return;}
  controls.hidden=false;
  const pageLabel=currentLang==='ro'?`Pagina ${latestGithubPage+1} din ${totalPages}`:`Page ${latestGithubPage+1} of ${totalPages}`;
  const moreLabel=currentLang==='ro'?'Mai multe':'More',isMorePage=latestGithubPage>0;
  controls.innerHTML=`<div class="key-carousel-nav"><button type="button" class="key-carousel-btn" data-gh-prev ${latestGithubPage===0?'disabled':''} aria-label="Previous">‹</button><button type="button" class="key-carousel-btn" data-gh-next ${latestGithubPage>=totalPages-1?'disabled':''} aria-label="Next">›</button></div><div class="key-carousel-dots">${pages.map((_,i)=>`<button type="button" class="key-carousel-dot ${i===latestGithubPage?'active':''}" data-gh-dot="${i}" aria-label="Slide ${i+1}" aria-pressed="${i===latestGithubPage}"></button>`).join('')}</div><div class="gh-meta" style="margin-top:0;"><span>${isMorePage?`${moreLabel} · `:''}${pageLabel}</span></div>`;
  controls.querySelector('[data-gh-prev]')?.addEventListener('click',()=>{if(latestGithubPage===0)return; latestGithubPage--; renderLatestGithub();});
  controls.querySelector('[data-gh-next]')?.addEventListener('click',()=>{if(latestGithubPage>=totalPages-1)return; latestGithubPage++; renderLatestGithub();});
  controls.querySelectorAll('[data-gh-dot]').forEach(dot=>dot.addEventListener('click',()=>{latestGithubPage=Number(dot.dataset.ghDot||0); renderLatestGithub();}));
}
...
function updateFilterState(query,total){
  const filterStateEl=byId('filterState'),separatorEl=byId('statusSeparator'),resultsCountEl=byId('resultsCount');
  if(!filterStateEl||!separatorEl||!resultsCountEl) return;
  const filterName=activeFilter==='all'?'all':activeFilter,readableFilter=filterName.toUpperCase(),qv=query?`"${query}"`:'—';
  const text=t('filter_state').replace('{filter}',readableFilter).replace('{query}',qv);
  resultsCountEl.textContent=String(total);
  const statusBar=byId('libraryStatus'),resetBtn=byId('resetFiltersBtn');
  if(statusBar) statusBar.style.visibility=''; if(resetBtn) resetBtn.style.visibility='';
  if(total===0){filterStateEl.hidden=true; separatorEl.hidden=true; return;}
  filterStateEl.hidden=false; separatorEl.hidden=false; filterStateEl.textContent=text;
}
function applyFilters(){
  ...
  if(!filtered.length){dom.libraryGrid.hidden=true; dom.libraryFallback.classList.add('show'); updateFilterState(query,0); return;}
  dom.libraryGrid.hidden=false; dom.libraryFallback.classList.remove('show');
  const frag=document.createDocumentFragment(); filtered.forEach(project=>frag.appendChild(createLibraryCard(project))); dom.libraryGrid.appendChild(frag);
...
function render(){applyTranslations(); renderKeyProjects(); renderLatestGithub(); setupKeyProjectsCarousel(); if(dom.searchInput&&dom.libraryGrid&&dom.libraryFallback) applyFilters();}
function renderKeyCarouselDots(totalSlides){
  if(!dom.keyCarouselDots) return;
  dom.keyCarouselDots.innerHTML='';
  const frag=document.createDocumentFragment();
  for(let i=0;i<totalSlides;i++){const dot=document.createElement('button'); dot.type='button'; dot.className='key-carousel-dot'; dot.setAttribute('aria-label',`Slide ${i+1}`); dot.addEventListener('click',()=>setKeyCarouselSlide(i)); frag.appendChild(dot);}
  dom.keyCarouselDots.appendChild(frag);
}
function setKeyCarouselSlide(index){
  if(!dom.keyProjectsGrid) return;
  const cards=dom.keyProjectsGrid.querySelectorAll('.project-card'); if(!cards.length) return;
  ...
  (dom.keyCarouselDots?.querySelectorAll('.key-carousel-dot')||[]).forEach((dot,i)=>{const active=i===keyCarouselIndex; dot.classList.toggle('active',active); dot.setAttribute('aria-pressed',String(active));});
}
function setupKeyProjectsCarousel(){
  if(!dom.keyProjectsGrid) return;
  const cards=dom.keyProjectsGrid.querySelectorAll('.project-card'); if(!cards.length) return;
  keyCarouselIndex=0; renderKeyCarouselDots(cards.length); setKeyCarouselSlide(0);
}
function renderHeroPreviewChips(){
  if(!dom.heroPreviewLinks) return;
  dom.heroPreviewLinks.innerHTML='';
  const frag=document.createDocumentFragment();
  HERO_PREVIEW_SLIDES.forEach((slide,index)=>{const button=document.createElement('button'); button.type='button'; button.className='chip'; button.textContent=slide.title; button.setAttribute('aria-label',`Preview ${slide.title}`); button.addEventListener('click',()=>{setHeroPreviewSlide(index); restartHeroPreviewAutoplay();}); frag.appendChild(button);});
  dom.heroPreviewLinks.appendChild(frag);
}
function keepActiveChipInView(chip){
  const container=dom.heroPreviewLinks; if(!container||!chip) return;
  const cr=container.getBoundingClientRect(),ir=chip.getBoundingClientRect();
  const chipLeft=ir.left-cr.left+container.scrollLeft,chipRight=chipLeft+ir.width,visibleLeft=container.scrollLeft,visibleRight=visibleLeft+container.clientWidth;
...
function setHeroPreviewSlide(index){
  ...
  if(dom.heroPreviewUrl){const cleanUrl=slide.url.replace(/^https?:\/\//,''); dom.heroPreviewUrl.textContent=cleanUrl; dom.heroPreviewUrl.href=slide.url;}
  if(dom.heroPreviewCode) dom.heroPreviewCode.href=slide.codeUrl||slide.url;
  loadHeroPreviewFrame(slide);
  (dom.heroPreviewLinks?.querySelectorAll('.chip')||[]).forEach((chip,i)=>{const active=i===previewSlideIndex; chip.classList.toggle('active',active); chip.setAttribute('aria-pressed',String(active)); if(active) keepActiveChipInView(chip);});
}
function restartHeroPreviewAutoplay(){clearInterval(previewIntervalId); if(!previewAutoPlayAllowed||!heroPreviewIsVisible||HERO_PREVIEW_SLIDES.length<2) return; previewIntervalId=setInterval(()=>setHeroPreviewSlide(previewSlideIndex+1),HERO_PREVIEW_AUTOPLAY_MS);}
...
function cacheDom(){
  dom={
    searchInput:byId('searchInput'),libraryGrid:byId('libraryGrid'),libraryFallback:byId('libraryFallback'),filterButtons:[...document.querySelectorAll('[data-filter]')],resetFiltersBtn:byId('resetFiltersBtn'),
    keyProjectsGrid:byId('keyProjectsGrid'),keyCarouselPrev:byId('keyCarouselPrev'),keyCarouselNext:byId('keyCarouselNext'),keyCarouselDots:byId('keyCarouselDots'),
    heroPreviewLinks:byId('heroPreviewLinks'),heroPreviewTitle:byId('heroPreviewTitle'),heroPreviewHeading:byId('heroPreviewHeading'),heroPreviewMeta:byId('heroPreviewMeta'),heroPreviewDescription:byId('heroPreviewDescription'),heroPreviewType:byId('heroPreviewType'),heroPreviewUrl:byId('heroPreviewUrl'),heroPreviewCode:byId('heroPreviewCode'),heroPreviewOpen:byId('heroPreviewOpen'),heroPreviewOpenSecondary:byId('heroPreviewOpenSecondary'),heroPreviewFrame:byId('heroPreviewFrame'),heroPreviewFallback:byId('heroPreviewFallback'),heroPreviewHint:byId('heroPreviewHint'),heroPreviewSection:byId('heroPreviewSection'),
...
async function fetchProjectsFromRemote(){
  try{
    ...
    let extracted=[];
    for(const m of matches){try{const parsed=Function(`return ${m[1]}`)(); if(Array.isArray(parsed)&&parsed.length){extracted=parsed; break;}}catch{}}
...
function setActiveFilter(nextFilter){
  ...
  dom.filterButtons?.forEach(button=>{const isActive=String(button.dataset.filter||'all').toLowerCase()===activeFilter; button.classList.toggle('active',isActive); button.setAttribute('aria-pressed',String(isActive));});
  applyFilters();
}
function resetFilters(){activeFilter='all'; if(dom.searchInput) dom.searchInput.value=''; setActiveFilter('all');}
function bindFilterEvents(){
  dom.filterButtons?.forEach(button=>button.addEventListener('click',()=>setActiveFilter(button.dataset.filter||'all')));
  dom.resetFiltersBtn?.addEventListener('click',resetFilters);
  dom.searchInput?.addEventListener('input',()=>{clearTimeout(searchDebounceId); searchDebounceId=setTimeout(applyFilters,180);});
}
...
function bindHeroPreviewEvents(){
  ...
  dom.heroPreviewFrame?.addEventListener('load',()=>{clearTimeout(heroPreviewFrameErrorTimer); toggleHeroPreviewFallback(false);});
  ...
  if('IntersectionObserver'in window&&dom.heroPreviewSection){
    new IntersectionObserver(entries=>{heroPreviewIsVisible=!!entries[0]?.isIntersecting; heroPreviewIsVisible?restartHeroPreviewAutoplay():stopHeroPreviewAutoplay();},{threshold:.35}).observe(dom.heroPreviewSection);
  }
  restartHeroPreviewAutoplay();
}
function bindLangToggle(){
  dom.langToggle?.addEventListener('click',()=>{currentLang=currentLang==='ro'?'en':'ro'; localStorage.setItem('portfolio-lang',currentLang); render(); setHeroPreviewSlide(previewSlideIndex);});
}
function bindGlobalEvents(){
  window.addEventListener('visibilitychange',()=>document.hidden?stopHeroPreviewAutoplay():restartHeroPreviewAutoplay());
  window.addEventListener('beforeunload',()=>{stopHeroPreviewAutoplay(); clearTimeout(heroPreviewFrameErrorTimer); clearTimeout(searchDebounceId);});
}
